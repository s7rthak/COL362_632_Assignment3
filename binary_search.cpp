...
void binary_search (FileHandler* fh_read, FileHandler* fh_write, int search_for, int &write_page, int &write_offset, int last_page_num, int num_of_int_each_page) {
    ...
    while (l_range != r_range) {
        ...
        PageHandler read = fh_read->PageAt(mid);
        char* read_data = read.GetData();
        ...
        fh_read->UnpinPage(mid);
    }
    
    bool cont = true;
    int cur_page = l_range;
    while (cur_page <= last_page_num && cont) {
        PageHandler read = fh_read->PageAt(cur_page);
        char* read_data = read.GetData();
...
int main (int argc, char* argv[]) {
    ...
    while (query >> search) {
        if (search == "SEARCH") {
            ...
            binary_search(&fh_read, &fh_write, to_search_num, write_page, write_offset, last_page_num, num_of_int_each_page);
        }
    }
